...
import { type LiverEvent, fetchLiverEventList } from "@/services/api";
...
export const useEventListStore = defineStore("eventListStore", () => {
	...
	const dateSectionList = computed(() => {
		return createDateSectionList(filteredEventList.value).filter(
			(dateSection) => {
				return dateSection.timeSectionList.some(
		...
		bookmarkStore.bookmarkEventMap.forEach((bookmarkType, id) => {
			...
			if (Notification.permission === "granted") {
				...
				const notification = new Notification(
					`${hhmm} ${bookmarkEvent.talent.name}`,
					{
	...
	function updateAddedEventList(newLiverEventList: LiverEvent[]) {
		...
		const newLiverEventIdSet = new Set(
			newLiverEventList.map((event) => event.id),
		);
		const prevLiverEventIdSet = new Set(
			liverEventList.value.map((event) => event.id),
		);

		const idDiffSet = newLiverEventIdSet.difference(prevLiverEventIdSet);

		const now = Date.now();
		const newAddedEventIdList: AddedEventId[] = Array.from(idDiffSet).map(
			(id) => {
				return {
					id,
					addedTime: now,

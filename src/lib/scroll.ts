...
export function scrollToCurrentTime(
	...
	const timeSectionList = dateSectionList.flatMap(
		(section) => section.timeSectionList,
	);
	// 現在時刻より後のtimeSectionを取得
	const afterNowIndex = timeSectionList.findIndex(
		(timeSection) => timeSection.time > now,
	);
	...
	const targetTimeSection =
		afterNowIndex > 0
			...
			: timeSectionList
					.filter((timeSection) => timeSection.events.length > 0)
					.at(-1);
...
export function scrollToSectionTop(time: number) {
	const targetSectionEl = document.querySelector(
		`[data-id="date-section"][data-time="${time}"]`,
	);

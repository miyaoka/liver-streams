...
export const useLiverEvent = (liverEvent: Ref<LiverEvent>) => {
  ...
  const isHovered = computed(() => {
    if (!hoverStore.hoveredTalent) return false;

    // 自身がホバー中のタレントか
    if (hoverStore.hoveredTalent === liverEvent.value.talent.name) return true;

    // ホバー中のタレントがコラボタレントに含まれているか
    const collaboTalentSet = toRaw(liverEvent.value.collaboTalentSet);
    if (collaboTalentSet.has(hoverStore.hoveredTalent)) return true;

    // ホバー中のコラボタレントにタレントが含まれているか
    return hoverStore.hoveredCollaboTalentSet.has(liverEvent.value.talent.name);
  });
